# Equipment Encyclopedia

| | |
|---|---|
| [Weapons](weapons.md) | Weapons, their properties, and how to use them. |
| [Armor](armor.md) | Armor, shields, and their requirements. |
| [Adventuring Gear](gear.md) | Miscellaneous supplies and packs for exploration. |
| [Trades](trades.md) | Gaming Sets, Musical Instruments, and Artisan's Tools. |

## Starting Equipment

Your Class and Background determine your starting equipment and gold.  You can purchase additional equipment for their listed Value.

### Starting at Higher Levels

If you start above Level **1**, you also gain the following equipment based on your starting Level:

| Starting Level | Gold | Magic Items |
|---|---|---|
| 2 – 4 | [LV × 50] gold | 1 Common |
| 5 – 10 | [(LV × 50) + 500] gold | 1 Common, 1 Uncommon |
| 11 – 16 | [(LV × 250) + 5,000] gold | 2 Common, 3 Uncommon, 1 Rare |
| 17 – 20 | [(LV × 250) + 20,000] gold | 2 Common, 4 Uncommon, 3 Rare, 1 Very Rare |

## Using Equipment

Equipment usually requires wearing or wielding it properly.  An item's type determines how it should be worn, and how many items of the same type a typical Humanoid can wear at the same time.

###### Item Types

| Type | Usage | Limit | Crafting Tool |
|---|---|---|---|
| Armor | Donned | One | Leatherworker's Tools, Smith's Tools, or Weaver's Tools |
| Clothing | Varies by type | Varies by type | Varies by type |
| Potion | Held in hand | - | Alchemist's Supplies or Herbalism Kit |
| Scroll | Held in hand | - | Calligrapher's Supplies |
| Shield | Donned | One | Leatherworker's Tools, Smith's Tools, or Woodcarver's Tools |
| Spellcasting Focus | Held in hand | One per hand | Jeweler's Tools, Painter's Supplies, or Woodcarver's Tools |
| Weapon | Held in hand | One per hand | Leatherworker's Tools, Smith's Tools, Tinker's Tools, or Woodcarver's Tools |
| Other | Varies | Varies | Tinker's Tools or another appropriate Artisan's Tool |

### Potions

A Potion can be consumed by you or administered to a creature within Reach. Either use requires the same action.

###### Clothing Types

| Type | Usage | Limit | Crafting Tool |
|---|---|---|---|
| Boots | On feet | One pair | Cobbler's Tools |
| Bracers | On forearm | One pair | Leatherworker's Tools or Smith's Tools |
| Cloak | On shoulders | One | Leatherworker's Tools or Weaver's Tools |
| Clothes | On body | One set | Weaver's Tools |
| Gloves | On hands | One pair | Leatherworker's Tools |
| Hat | On head | One | Leatherworker's Tools or Weaver's Tools |
| Jewelry | Varies | - | Glassblower's Tools, Jeweler's Tools, or Tinker's Tools |

## Crafting

You can craft your own equipment using tools, time, and raw materials.

### Requirements

You can craft Weapons, Armor, and Adventuring Gear, provided you have Proficiency with the Artisan's Tool associated with the item and the tool is appropriate to the material used.

### Time and Cost

The Cost of crafting an item is paid in raw materials and time based on the item's Value.  You use half the item's Value in raw materials, and it takes **1** day to craft an item for every **25** gold it's worth.

Every day of crafting requires **8** hours of work, but the total hours and days don't need to be consecutive.

You can work with other characters to shorten the crafting time, provided each character has the appropriate Proficiencies.  Each character's contribution reduces the total time as though they were working independently.

## Materials

Items made with special Materials may gain additional properties.  You must acquire a Material before you can craft an item with it.  Crafting an item with a Material increases the item's Value, but doesn't increase the time required to craft the item.

--8<-- "equipment/_materials/adamantine.md"

--8<-- "equipment/_materials/heartwood.md"

--8<-- "equipment/_materials/ironwill.md"

--8<-- "equipment/_materials/mithral.md"

--8<-- "equipment/_materials/quicksilver.md"

--8<-- "equipment/_materials/silver.md"

## Masterwork

Items of superior quality are more effective than their normal versions.  There are different types of Masterwork bonuses, and each bonus is added to an item individually.

### Value

The Value of a Masterwork item is determined by each of the item's Masterwork bonuses, in addition to the item's base Value.  For Armor Class bonuses on a set of Armor _(not a Shield)_, use the Armor column instead.

| Bonus | Value | Armor | Min. Level |
|---|---|---|---|
| +1 | +400 gold | +4,000 gold | 5 |
| +2 | +4,000 gold | +40,000 gold | 11 |
| +3 | +40,000 gold | +400,000 gold | 17 |

### Crafting

Crafting a Masterwork item costs half as much as purchasing one, but requires Expertise in the corresponding Artisan's Tools and a minimum Character Level based on the item's bonus.

### Physical Attack

Masterwork items with a Physical Attack bonus add that bonus to Physical Attack and Damage Rolls made using the item.

### Spell Attack

Masterwork items with a Spell Attack bonus add that bonus to Spell Attack Rolls made using the item as a Spellcasting Focus _(you must be able to use the item as a Spellcasting Focus normally)_.

### Spell Save

Masterwork items with a Spell Save bonus add that bonus to the Spell Save DC of Spells cast using the item as a Spellcasting Focus _(you must be able to use the item as a Spellcasting Focus normally)_.

### Armor Class

Masterwork items with an Armor Class bonus add that bonus to the item's Armor Class bonus _(an item that doesn't normally affect Armor Class doesn't gain this benefit)_.
