...
The time it takes to craft a nonmagical item is based on the item's worth. For every 25 GP, it takes a day to craft an item.

Every day of crafting requires 8 hours of work, but the total hours and days don't need to be consecutive.

Characters can work together to shorten the crafting time, provided each character meet's the item's tool requirements. Divide the time required by the number of characters working on the item.
...
When a physical attack with this weapon hits a shapeshifted creature, it deals one additional die of damage.
...
If an item has multiple masterwork bonuses, each one is added to the item's total worth individually.

When adding masterwork armor class to a set of armor, other than a shield, use the Armor Cost column instead.

| bonus | cost | armor cost |
|---|---|---|
