### Pyrotechnics
2nd-Level Transmutation
{ .subtitle }

| | |
|---|---|
| Classes | Artificer, Bard, Sorcerer, Wizard |
| Components | V, S |
| Casting Time | Normal Action |
| Range | 60 ft. |
| Target | Everything in a 20-ft. radius Sphere |
| Duration | 1 minute (Concentration) |

##### Spell Effect
You create a burst of flames, then immediately extinguish any fire in the area. Choose one of the following effects.

| | |
|---|---|
| Fireworks | Each target makes a Fortitude Save.  On a failed save, you deal deal **2d6** Fire Damage, and you blind the target until the end of its next Turn.  On a success, you deal half as much damage. |
| Smoke | The area is filled with smoke and Heavily Obscured. This Spell ends if the smoke is dispersed. |

##### At Higher Levels
Add **1d6** damage and **10** feet to the radius per additional Spell Level.
