# Class Codex

Your class reflects your specialized training and defines how you interact with the world as an adventurer.  Each class also has subclasses that further enhance your capabilities.

| Class | |
|---|---|
| [Artificer](artificer/index.md) | Magical craftspeople who infuse mundane objects with arcane power. |
| [Barbarian](barbarian/index.md) | Fierce warriors who channel primal rage for devastating effect in combat. |
| [Bard](bard/index.md) | Versatile performers who weave magic through music, words, and wit. |
| [Cleric](cleric/index.md) | Divine champions who call upon their deity's power to heal and smite. |
| [Druid](druid/index.md) | Nature's guardians who shapeshift and command the elements. |
| [Fighter](fighter/index.md) | Disciplined warriors trained in a broad spectrum of combat techniques. |
| [Monk](monk/index.md) | Masters of unarmed combat who focus body and mind into a lethal instrument. |
| [Paladin](paladin/index.md) | Holy warriors who combine martial prowess with divine oaths and magic. |
| [Ranger](ranger/index.md) | Skilled hunters and trackers who forge bonds with beasts and the wilderness. |
| [Rogue](rogue/index.md) | Cunning opportunists who strike from the shadows with deadly precision. |
| [Sorcerer](sorcerer/index.md) | Spellcasters whose magic flows from innate power within their bloodline. |
| [Warlock](warlock/index.md) | Those who forge pacts with powerful entities to gain forbidden knowledge. |
| [Wizard](wizard/index.md) | Scholars who master arcane magic through rigorous study and practice. |

## Leveling Up
When you gain a level, choose one of your classes to gain that level in, or you can choose a secondary class if you don't have one yet.  You gain the chosen class’s corresponding features for that level.

### Hit Dice and Hit Points
When you gain a level in a class, you gain one of that class’s hit dice.  Add that hit die’s maximum plus your Constitution to your hit point maximum.

### Primary Class
The class you choose at level 1 is your Primary Class.  When you gain your first level in your primary class, you gain that class's starting equipment and primary proficiencies, choose a class ability (if needed), then increase that class ability by 1.

### Secondary Class
You can take levels in two classes.  When you gain your first level in your secondary class, you gain the chosen class’s secondary proficiencies.

Once you choose a secondary class, you cannot change it, and you cannot take levels in a third class.

### Character Level
Your character level is equal to the sum of your primary class’s levels and your secondary class’s levels.  Your character level determines your proficiency bonus, your ability maximum, and gives an additional feat and/or bonus to an ability of your choice.

| Character Level | Proficiency Bonus | Ability Max | Benefit |
|---|---|---|---|
| 1 | +2 | 4 | - |
| 2 | +2 | 4 | +1 ability |
| 3 | +2 | 4 | feat |
| 4 | +2 | 4 | +1 ability |
| 5 | +3 | 5 | +1 ability + feat |
| 6 | +3 | 5 | feat |
| 7 | +3 | 5 | +1 ability |
| 8 | +3 | 5 | +1 ability |
| 9 | +4 | 5 | feat |
| 10 | +4 | 5 | +1 ability |
| 11 | +4 | 6 | +1 ability + feat |
| 12 | +4 | 6 | feat |
| 13 | +5 | 6 | +1 ability |
| 14 | +5 | 6 | +1 ability |
| 15 | +5 | 6 | feat |
| 16 | +5 | 6 | +1 ability |
| 17 | +6 | 7 | +1 ability + feat |
| 18 | +6 | 7 | feat |
| 19 | +6 | 7 | +1 ability |
| 20 | +6 | 7 | epic boon |

### Multiclass Spellcasting
If you gain the Spellcasting feature from both your primary and secondary classes, your capacity for spell slots depends on your individual levels in those classes.  Spellcasting variants, such as Pact Magic and Sorcery, do not count towards multiclass spellcasting.

| Class | Multiplier |
|---|---| 
| Artificer | × ½ |
| Bard | × 1 |
| Cleric | × 1 |
| Druid | × 1 |
| Fighter | × ⅓ |
| Paladin | × ½ |
| Ranger | × ½ |
| Rogue | × ⅓ |
| Wizard | × 1 |

Each class has a different capacity for spellcasting.  To determine your available spell slots, multiply each of your class's levels as shown above, add them together, and consult the table below.

| Spellcaster Level | 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 | 9 |
|---|---|---|---|---|---|---|---|---|---|
| 1 | 2 | - | - | - | - | - | - | - | - |
| 2 | 3 | - | - | - | - | - | - | - | - |
| 3 | 4 | 2 | - | - | - | - | - | - | - |
| 4 | 4 | 3 | - | - | - | - | - | - | - |
| 5 | 4 | 3 | 2 | - | - | - | - | - | - |
| 6 | 4 | 3 | 3 | - | - | - | - | - | - |
| 7 | 4 | 3 | 3 | 1 | - | - | - | - | - |
| 8 | 4 | 3 | 3 | 2 | - | - | - | - | - |
| 9 | 4 | 3 | 3 | 3 | 1 | - | - | - | - |
| 10 | 4 | 3 | 3 | 3 | 2 | - | - | - | - |
| 11 | 4 | 3 | 3 | 3 | 2 | 1 | - | - | - |
| 12 | 4 | 3 | 3 | 3 | 2 | 1 | - | - | - |
| 13 | 4 | 3 | 3 | 3 | 2 | 1 | 1 | - | - |
| 14 | 4 | 3 | 3 | 3 | 2 | 1 | 1 | - | - |
| 15 | 4 | 3 | 3 | 3 | 2 | 1 | 1 | 1 | - |
| 16 | 4 | 3 | 3 | 3 | 2 | 1 | 1 | 1 | - |
| 17 | 4 | 3 | 3 | 3 | 2 | 1 | 1 | 1 | 1 |
| 18 | 4 | 3 | 3 | 3 | 3 | 1 | 1 | 1 | 1 |
| 19 | 4 | 3 | 3 | 3 | 3 | 2 | 1 | 1 | 1 |
| 20 | 4 | 3 | 3 | 3 | 3 | 2 | 2 | 1 | 1 |

You prepare your spells for each class individually.  Each spell you prepare is associated with the class you used to prepare it, and you use that class’s spellcasting ability to cast it.

## Class Abilities
Every class has a class ability that is used to calculate the save DCs, spellcasting ability, and how many times you can use some features.  Class abilities are usually referred to by the class their associated with (e.g. Fighter Ability, Wizard Ability).
If a class has a choice of Class Ability, you choose the ability when you take your first level in that class.

### Secondary Abilities
Secondary abilities are just like class abilities, though not every class has a secondary ability.

| Class | Class Ability | Secondary Ability |
|---|---|---|
| Artificer | intelligence | - |
| Barbarian | strength | constitution |
| Bard | intelligence or charisma | - |
| Cleric | intelligence, wisdom, or charisma | - |
| Druid | intelligence or wisdom | - |
| Fighter | strength or dexterity | determined by subclass |
| Monk | dexterity | intelligence or wisdom |
| Paladin | strength | charisma |
| Ranger | strength or dexterity | intelligence or wisdom |
| Rogue | dexterity | determined by subclass |
| Sorcerer | wisdom  or charisma | - |
| Warlock | intelligence or charisma | - |
| Wizard | intelligence | - |