# Item Materials

Items made with special Materials may gain additional properties.

| Material | Value | Description |
|---|---|---|
| [Adamantine](#adamantine) | 500 gold | Immune to Physical damage from lesser sources; ignores Critical Hits. |
| [Heartwood](#heartwood) | 1,000 gold | Self-mending wood that serves as a Spellcasting Focus. |
| [Ironwill](#ironwill) | 2,500 gold | A Focus that lets you cast and fight with your Spellcasting Ability. |
| [Mithral](#mithral) | 500 gold | Lightweight metal that eases Armor and lightens Weapons. |
| [Quicksilver](#quicksilver) | 500 gold | Reshapes at will to don Armor or reforge a Weapon. |
| [Silver](#silver) | 100 gold | An alchemical coating that harms shapeshifters. |

--8<-- "item/material/adamantine.md"

--8<-- "item/material/heartwood.md"

--8<-- "item/material/ironwill.md"

--8<-- "item/material/mithral.md"

--8<-- "item/material/quicksilver.md"

--8<-- "item/material/silver.md"
