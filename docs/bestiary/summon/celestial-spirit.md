---
label: Summon Celestial
source: /spell/level/5/summon-celestial.md
---

# Celestial Spirit
A **Celestial Spirit** is a Companion conjured by the [Summon Celestial](/spell/level/5/summon-celestial.md) spell.
{ .subtitle }

### Celestial Spirit: Avenger
Large Celestial, Neutral
{ .subtitle }

| Defenses | |
|---|---|
| HP | **65** _[(10 + CON) × Spell Level]_ |
| Hit Dice | 1d10 per Spell Level |
| AC | **16 + PB** _[9 + DEX + Spell Level + PB]_ |
| Armor | Natural |
| Resistance | Radiant |
| Condition Immunity | Charm, Fear |

| Abilities | | | | | |
|---|---|---|---|---|---|
| **STR** | +3 | **DEX** | +2 | **CON** | +3 |
| **INT** | +0 | **WIS** | +2 | **CHA** | +3 |

| Traits | |
|---|---|
| Languages | Celestial; Understands the Languages you know |
| Senses | Darkvision (60 ft.) |
| Speed | 30 ft., Fly 40 ft. |
| Reach | 5 ft. |

| Features | |
|---|---|
| Multiattack | The spirit makes **Spell Level ÷ 2** attacks, rounded down. |

#### Actions
| Avenger's Bow | Ranged Attack |
|---|---|
| Range | 600 ft. |
| Attack | **+Spell Attack** to hit |
| Damage | **2d6 + 7** Radiant Damage _[2d6 + DEX + Spell Level]_ |

| Healing Touch | Normal Action, 1 use per Long Rest |
|---|---|
| | One creature within Reach heals by **2d8 + Spell Level**. |


### Celestial Spirit: Defender
Large Celestial, Neutral
{ .subtitle }

| Defenses | |
|---|---|
| HP | **65** _[(10 + CON) × Spell Level]_ |
| Hit Dice | 1d10 per Spell Level |
| AC | **18 + PB** _[11 + DEX + Spell Level + PB]_ |
| DR | -STR |
| Armor | Natural |
| Resistance | Radiant |
| Condition Immunity | Charm, Fear |

| Abilities | | | | | |
|---|---|---|---|---|---|
| **STR** | +3 | **DEX** | +2 | **CON** | +3 |
| **INT** | +0 | **WIS** | +2 | **CHA** | +3 |

| Traits | |
|---|---|
| Languages | Celestial; Understands the Languages you know |
| Senses | Darkvision (60 ft.) |
| Speed | 30 ft., Fly 40 ft. |
| Reach | 5 ft. |

| Features | |
|---|---|
| Multiattack | The spirit makes **Spell Level ÷ 2** attacks, rounded down. |

#### Actions
| Defender's Mace | Melee Attack |
|---|---|
| Range | Reach |
| Attack | **+Spell Attack** to hit |
| Damage | **1d10 + 8** Radiant Damage _[1d10 + STR + Spell Level]_ |
| Effect | After this attack deals damage, one creature within **10** feet gains **1d10** Temporary Hit Points. |

| Healing Touch | Normal Action, 1 use per Long Rest |
|---|---|
| | One creature within Reach heals by **2d8 + Spell Level**. |
