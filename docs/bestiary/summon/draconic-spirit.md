---
label: Summon Dragon
source: /spell/level/5/summon-dragon.md
---

# Draconic Spirit
A **Draconic Spirit** is a Companion conjured by the [Summon Dragon](/spell/level/5/summon-dragon.md) spell.
{ .subtitle }

### Draconic Spirit
Large Dragon, Neutral
{ .subtitle }

| Defenses | |
|---|---|
| HP | **65** _[(10 + CON) × Spell Level]_ |
| Hit Dice | 1d10 per Spell Level |
| AC | **19 + PB** _[12 + DEX + Spell Level + PB]_ |
| DR | **1** _[STR (max 1)]_ |
| Armor | Natural (Medium) |
| Resistance | Acid, Cold, Fire, Lightning, Poison |
| Condition Immunity | Charm, Fear, Sickness |

| Abilities | | | | | |
|---|---|---|---|---|---|
| **STR** | +4 | **DEX** | +2 | **CON** | +3 |
| **INT** | +0 | **WIS** | +2 | **CHA** | +2 |

| Traits | |
|---|---|
| Languages | Draconic; Understands the Languages you know |
| Senses | Blindsight (30 ft.), Darkvision (60 ft.) |
| Speed | 30 ft., Fly 60 ft., Swim 30 ft. |
| Reach | 10 ft. |

| Features | |
|---|---|
| Shared Resistance | After you conjure the spirit, choose **1** of its Resistances.  While the spirit is conjured, you gain the chosen Resistance and its Breath Weapon deals that Damage Type. |
| Multiattack | The spirit makes **Spell Level ÷ 2** attacks, rounded down, and uses Breath Weapon. |

#### Actions
| Rend | Melee Attack |
|---|---|
| Range | Reach |
| Attack | **+Spell Attack** to hit |
| Damage | **1d6 + 9** Piercing Damage _[1d6 + STR + Spell Level]_ |

| Breath Weapon | Normal Action |
|---|---|
| | Everything in a **30**-foot Cone makes a Reflex Save _(DC **Spell Save DC**)_; on a failure, you deal **2d6 + Spell Level** damage of the chosen Shared Resistance type, or half as much on a success. |
