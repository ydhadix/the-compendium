---
label: Tiny Servant
source: /spell/level/3/tiny-servant.md
---

# Animated Servant
An Animated Servant is a nonmagical object brought to life by the [Tiny Servant](/spell/level/3/tiny-servant.md) spell.
{ .subtitle }

### Animated Servant
Tiny Construct, Unaligned
{ .subtitle }

| Defenses | | | |
|---|---|---|---|
| HP | 10 | **Hit Dice** | 1d4 per Spell Level |
| AC | 10 + DEX + PB | **DR** | 0 |
| Armor | Natural | **Resistance** | — |
| Protection | — | **Immunity** | Poison, Psychic; Blindness, Charm, Deafness, Exhaustion, Fear, Sickness, Paralysis, Petrification |

| Abilities | | | | | |
|---|---|---|---|---|---|
| STR | -3 | **DEX** | +3 | **CON** | +0 |
| INT | -4 | **WIS** | +0 | **CHA** | -5 |

| Traits | |
|---|---|
| Languages | Understands the Languages you know, but can't speak |
| Senses | Blindsight 60 ft. (blind otherwise) |
| Speed | 30 ft., Climb 30 ft. |
| Reach | 5 ft. |

#### Actions
| Slam | Melee Attack |
|---|---|
| Range | Reach |
| Attack | **+your Spell Attack** to hit |
| Damage | **1d4 + DEX** Bludgeoning Damage. |
