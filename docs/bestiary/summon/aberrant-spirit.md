---
label: Summon Aberration
source: /spell/level/4/summon-aberration.md
---

# Aberrant Spirit
An Aberrant Spirit is a Companion conjured by the [Summon Aberration](/spell/level/4/summon-aberration.md) spell.
{ .subtitle }

| Variants |
|---|
| [Beholderkin](#aberrant-spirit-beholderkin) |
| [Mind Flayer](#aberrant-spirit-mind-flayer) |
| [Slaad](#aberrant-spirit-slaad) |

### Aberrant Spirit: Beholderkin
Medium Aberration, Neutral
{ .subtitle }

| Defenses | | | |
|---|---|---|---|
| HP | (8 + CON) × Spell Level | **Hit Dice** | 1d8 per Spell Level |
| AC | 10 + DEX + PB + Spell Level | **DR** | — |
| Armor | Natural | **Resistance** | — |
| Protection | — | **Immunity** | Psychic |

| Abilities | | | | | |
|---|---|---|---|---|---|
| STR | +3 | **DEX** | +0 | **CON** | +2 |
| INT | +3 | **WIS** | +0 | **CHA** | -2 |

| Traits | |
|---|---|
| Languages | Deep Speech; Understands the Languages you know |
| Senses | Darkvision (60 ft.) |
| Speed | 30 ft., Hover 30 ft. |
| Reach | 5 ft. |

| Features | |
|---|---|
| Multiattack | The spirit makes **Spell Level ÷ 2** attacks, rounded down. |

#### Actions
| Eye Ray | Ranged Attack |
|---|---|
| Range | 150 ft. |
| Attack | **+your Spell Attack** to hit |
| Damage | **1d8 + INT + Spell Level** Psychic Damage |

### Aberrant Spirit: Mind Flayer
Medium Aberration, Neutral
{ .subtitle }

| Defenses | | | |
|---|---|---|---|
| HP | (8 + CON) × Spell Level | **Hit Dice** | 1d8 per Spell Level |
| AC | 10 + DEX + PB + Spell Level | **DR** | — |
| Armor | Natural | **Resistance** | — |
| Protection | — | **Immunity** | Psychic |

| Abilities | | | | | |
|---|---|---|---|---|---|
| STR | +3 | **DEX** | +0 | **CON** | +2 |
| INT | +3 | **WIS** | +0 | **CHA** | -2 |

| Traits | |
|---|---|
| Languages | Deep Speech; Understands the Languages you know |
| Senses | Darkvision (60 ft.) |
| Speed | 30 ft. |
| Reach | 5 ft. |

| Features | |
|---|---|
| Whispering Aura | After the spirit's Turn starts, each creature in a 5-foot Aura makes a Will Save; on a failure, the spirit deals **2d6** Psychic Damage. |
| Multiattack | The spirit makes **Spell Level ÷ 2** attacks, rounded down. |

#### Actions
| Psychic Slam | Melee Attack |
|---|---|
| Range | Reach |
| Attack | **+your Spell Attack** to hit |
| Damage | **1d8 + STR + Spell Level** Psychic Damage |

### Aberrant Spirit: Slaad
Medium Aberration, Neutral
{ .subtitle }

| Defenses | | | |
|---|---|---|---|
| HP | (8 + CON) × Spell Level | **Hit Dice** | 1d8 per Spell Level |
| AC | 10 + DEX + PB + Spell Level | **DR** | — |
| Armor | Natural | **Resistance** | — |
| Protection | — | **Immunity** | Psychic |

| Abilities | | | | | |
|---|---|---|---|---|---|
| STR | +3 | **DEX** | +0 | **CON** | +2 |
| INT | +3 | **WIS** | +0 | **CHA** | -2 |

| Traits | |
|---|---|
| Languages | Deep Speech; Understands the Languages you know, but can't speak |
| Senses | Darkvision (60 ft.) |
| Speed | 30 ft. |
| Reach | 5 ft. |

| Features | |
|---|---|
| Regeneration | After the spirit's Turn starts, it heals by **5**. |
| Multiattack | The spirit makes **Spell Level ÷ 2** attacks, rounded down. |

#### Actions
| Claw | Melee Attack |
|---|---|
| Range | Reach |
| Attack | **+your Spell Attack** to hit |
| Damage | **1d10 + STR + Spell Level** Slashing Damage |
| Effect | After this attack deals Damage, the target target can't heal until the start of the spirit's next Turn. |
