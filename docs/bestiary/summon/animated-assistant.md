---
label: Artificer
source: /character/class/artificer/index.md#1-tradesman
---

# Animated Assistant
An Animated Assistant is a mechanical helper created by the Artificer's [Tradesman](/character/class/artificer/index.md#1-tradesman) feature.  These compact assistants are designed to help with storing, crafting, and infusing items.
{ .subtitle }

### Animated Assistant
Tiny Construct, Neutral
{ .subtitle }

| Defenses | | | |
|---|---|---|---|
| HP | (4 + CON) × Level | **Hit Dice** | 1d4 per Level |
| AC | 10 + DEX + PB | **DR** | — |
| Armor | Natural | **Resistance** | — |
| Protection | — | **Immunity** | Exhaustion, Poison, Psychic, Sickness |

| Abilities | | | | | |
|---|---|---|---|---|---|
| STR | −3 | **DEX** | +2 | **CON** | +1 |
| INT | +0 | **WIS** | +0 | **CHA** | −2 |

| Traits | |
|---|---|
| Languages | — |
| Senses | Darkvision 60 ft. |
| Speed | 20 ft., Hover 30 ft. |
| Reach | 5 ft. |

| Features | |
|---|---|
| Mobile Storage | The assistant can store and quickly retrieve items up to **Artificer Ability**. |
| Crafting Aid | The assistant can help you craft items without meeting the item's requirements. |
| Evasion | After the assistant makes a Reflex Save to avoid damage, it takes half damage on a failure and no damage on a success. |

#### Actions
| Force Strike | Melee or Ranged Attack |
|---|---|
| Range | Reach / 30 ft. |
| Attack | **+your Spell Attack** to hit |
| Damage | **1d6 + DEX** Force Damage |