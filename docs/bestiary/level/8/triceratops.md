# Triceratops
**Triceratops** are built around a bone shield and three horns, and they charge anything that crowds the herd.
{ .subtitle }

### Triceratops
Huge Beast, Unaligned
{ .subtitle }

| Defenses | |
|---|---|
| HP | **135** _[(12 + CON) × (Level + 1)]_ |
| Hit Dice | 8d12 |
| AC | **14** _[12 + DEX + PB]_ |
| DR | **1** _[STR (max 1)]_ |
| Armor | Natural (Medium) |

| Abilities | | | | | |
|---|---|---|---|---|---|
| **STR** | +6 | **DEX** | -1 | **CON** | +3 |
| **INT** | -4 | **WIS** | +0 | **CHA** | -3 |

| Traits | |
|---|---|
| Languages | — |
| Senses | — |
| Speed | 50 ft. |
| Reach | 5 ft. |
| Level | 8 |
| Proficiency Bonus | +3 |

| Features | |
|---|---|
| Multiattack | The triceratops makes **2** Gore attacks. |

#### Actions
| Gore | Melee Attack |
|---|---|
| Range | Reach |
| Attack | **+9** to hit _[STR + PB]_ |
| Damage | **2d12 + 6** Piercing Damage _[2d12 + STR]_ |
| Effect | Before this attack deals damage, if the triceratops moved at least **20** feet in a straight line toward a Huge or smaller target, it deals an additional **2d8** damage.  After dealing this damage, the triceratops knocks the target Prone. |
