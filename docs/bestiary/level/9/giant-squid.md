# Giant Squid
**Giant Squid** rise out of the deep to pull crews off decks, and drag what they take straight back down.
{ .subtitle }

### Giant Squid
Huge Beast, Unaligned
{ .subtitle }

| Defenses | |
|---|---|
| HP | **130** _[(12 + CON) × (Level + 1)]_ |
| Hit Dice | 9d12 |
| AC | **14** _[8 + DEX + PB]_ |
| Armor | Unarmored |

| Abilities | | | | | |
|---|---|---|---|---|---|
| **STR** | +6 | **DEX** | +2 | **CON** | +1 |
| **INT** | -3 | **WIS** | +0 | **CHA** | -3 |

| Traits | |
|---|---|
| Languages | — |
| Senses | Darkvision (120 ft.) |
| Speed | 5 ft., Swim 80 ft. |
| Reach | 5 ft. |
| Saving Throws | Fortitude, Reflex |
| Skills | Perception |
| Level | 9 |
| Proficiency Bonus | +4 |

| Features | |
|---|---|
| Water Breathing | The squid can breathe only underwater. |
| Multiattack | The squid makes one Bite attack and one Tentacle attack. |

#### Actions
| Bite | Melee Attack |
|---|---|
| Range | Reach |
| Attack | **+10** to hit _[STR + PB]_ |
| Damage | **4d10 + 6** Piercing Damage _[4d10 + STR]_ |

| Tentacle | Melee Attack |
|---|---|
| Range | 15 ft. |
| Attack | **+10** to hit _[STR + PB]_ |
| Damage | **3d8 + 6** Bludgeoning Damage _[3d8 + STR]_ |
| Effect | After this attack deals damage, the squid grapples a Huge or smaller target in one of its two tentacles and pulls it up to **10** feet toward itself. |

| Reactions | |
|---|---|
| Ink Cloud | **1 use per Long Rest:** After the squid takes damage while underwater, it fills a **15**-foot Cube centered on itself with ink, Heavily Obscuring it, then Swims up to its Swim Speed. |
